...
static init_stage_t g_cleanup_stage = INIT_NONE; 
static socket_descriptor_t g_socket_descriptors = {-1, -1}; 
 ...
 */
static bool cleanup_server (init_stage_t stage);
...
server_init (const server_config_t * config)
{
    if (NULL == config)
    {
        ...
        cleanup_server (g_cleanup_stage);
        return false;
    }
    ...
    g_cleanup_stage = INIT_SIGNALS;
    syslog_write (INFO, "Signal handlers initialized successfully");

    if (!socket_init (config, &g_socket_descriptors))
    {
    ...
    g_cleanup_stage = INIT_SOCKET;
    syslog_write (INFO, "Sockets initialized successfully");

    if (!poll_init (config, &g_socket_descriptors))
    {
...
server_run (void)
{
    ...
    if (!g_server_running || g_cleanup_stage != INIT_COMPLETE)
    {
        syslog_write (ERROR,
                      "Attempted to run server before complete initialization");
        return false;
    }
...
server_shutdown (void)
{
    if (!g_server_running)
    {
        syslog_write (WARNING,
                      "Attempted to shut down server that is not running");
        return false;
    }
    ...
    // Clean up from the stage reached which for shutdown is INIT_COMPLETE
    return cleanup_server (g_cleanup_stage);
}
...
cleanup_server (init_stage_t stage)
{
    ...
    // Perform cleanup in reverse order of initialization by calling
    // cleanup function up to the stage it reached and falling through
    ...
    switch (stage)
    {
        ...
        case INIT_POLL:
            ...
            {
                syslog_write (WARNING,
                              "Failed to cleanly shut down poll subsystem");
                success = false;
            }
            else
            {
        ...
        case INIT_SOCKET:
            if (!socket_cleanup (&g_socket_descriptors))
            {
        ...
        case INIT_SIGNALS:
            ...
            {
                syslog_write (WARNING,
                              "Failed to cleanly reset signal handlers");
                success = false;
            }
            else
            {
