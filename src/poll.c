...
bool
poll_init (const server_config_t *     p_config,
           const socket_descriptor_t * p_descriptors)
{
    ...
    g_poll_state.num_fds     = 2;
    g_poll_state.poll_fds
        = calloc (g_poll_state.num_fds, sizeof (struct pollfd));
    ...
    syslog_write (
        INFO, "Poll subsystem initialized with %zu fds", g_poll_state.num_fds);
    return true;
}
...
poll_cleanup (void)
{
    // Uninitialized module check
    if (!g_poll_state.initialized)
    {
        syslog_write (WARNING,
                      "Attempt to clean up uninitialized poll subsystem");
    }
...
handle_tcp_connection (int tcp_socket)
{
    ...
    {
        syslog_write (
            ERROR, "Error reading from TCP client: %s", strerror (errno));
        close (client_fd);
        return;
    }
    ...
    if (bytes_read > 0)
    {
        ...
        syslog_write (
            INFO,
            "Received format string from TCP client: '%s', length: %ld",
            buffer,
            bytes_read);
    }
    else
    {
        syslog_write (
            INFO,
            "Received empty format string from TCP client (bytes_read=%ld)",
            bytes_read);
    }

    // Format time according to request (or use default if empty)
    if (!format_time (
            bytes_read > 0 ? buffer : NULL, time_buffer, sizeof (time_buffer)))
    {
    ...
    syslog_write (
        INFO, "Sending formatted time to TCP client: '%s'", time_buffer);

    // Send formatted time to client
    if (send (client_fd, time_buffer, strlen (time_buffer), 0) < 0)
    {
        syslog_write (ERROR,
                      "Failed to send response to TCP client: %s",
                      strerror (errno));
    }
...
handle_udp_datagram (int udp_socket)
{
    ...
    // Receive datagram aand check MTU size
    bytes_read = recvfrom (udp_socket,
                           buffer,
                           sizeof (buffer) - 1,
                           0,
    ...
    {
        syslog_write (
            ERROR, "Error receiving UDP datagram: %s", strerror (errno));
        return;
    }

    // Log client request
    syslog_write (INFO,
                  "UDP request from %s:%d",
                  inet_ntoa (client_addr.sin_addr),
                  ntohs (client_addr.sin_port));
    ...
    if (bytes_read > 0)
    {
        ...
        if (bytes_read > FORMAT_DISPLAY_LENGTH)
        {
            ...
            syslog_write (INFO,
                          "Received long format string from UDP client: "
                          "'%s...' (truncated), length: %ld",
                          truncated,
                          bytes_read);
        }
        else
        {
            syslog_write (
                INFO,
                "Received format string from UDP client: '%s', length: %ld",
                buffer,
                bytes_read);
        }
    }
    else
    {
        syslog_write (
            INFO,
            "Received empty format string from UDP client (bytes_read=%ld)",
            bytes_read);
    }
    ...
    syslog_write (
        INFO, "Sending formatted time to UDP client: '%s'", time_buffer);
    ...
    {
        syslog_write (ERROR,
                      "Failed to send response to UDP client: %s",
                      strerror (errno));
    }
}
...
format_time (const char * p_format_str, char * p_output, size_t output_size)
{
    ...
    char         truncated_format[TRUNCATED_BUFFER_SIZE] = {0};
    ...
    {
        syslog_write (ERROR, "Poll subsystem parameters invalid");
        return false;
    }
    ...
    // Check for newline or only whitespace
    ...
    p_format = (NULL != p_format_str && p_format_str[0] != '\0'
                && !is_only_whitespace)
                ? p_format_str
    ...
    if (p_format_str != NULL && !is_only_whitespace)
    {
        ...
        if (format_len > MAX_FORMAT_LENGTH)
        {
            ...
            syslog_write (WARNING,
                          "Format string too long (%zu bytes), truncated",
                          format_len);
        }
        ...
        if (format_len > FORMAT_DISPLAY_LENGTH)
        {
            ...
            syslog_write (INFO,
                          "Using provided format (truncated): '%s...'",
                          display_format);
        }
        else
        {
    ...
    {
        syslog_write (INFO,
                      "Format string is NULL, using default format: '%s'",
                      g_poll_state.server_cfg->time_format);
    }
    else
    {
        syslog_write (
            INFO,
            "Format string is empty or whitespace, using default format: '%s'",
            g_poll_state.server_cfg->time_format);
    }
